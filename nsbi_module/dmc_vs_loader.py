"""
Shared loader for the driftdm_dmc_vs (DMC with var_start / alpha) model.
...
and instantiation so that every analysis script can use:
    ...
    from dmc_vs_loader import get_dmc_vs_model
    m = get_dmc_vs_model()

Model: DMC with 7 parameters (muc, b, non_dec, sd_non_dec, tau, A, alpha),
where alpha controls starting-point variability (Beta(alpha, alpha) distribution).
"""

from numba import njit
import numpy as np

# ── Shared Numba tool functions (copied from dmc_v2_loader) ──────────────
...
# ── Starting-point sampling (Beta via rejection) ─────────────────────────


@njit
def _sample_symmetric_beta(alpha):
    """Sample from Beta(alpha, alpha) using rejection sampling.

    Proposal distribution: Uniform(0, 1).
    Acceptance probability = (4*x*(1-x))^(alpha-1).

    For alpha in [2, 8], acceptance rate is ~32%–67%.
    """
    for _ in range(1000):
        x = np.random.random()
        if np.random.random() < (4.0 * x * (1.0 - x)) ** (alpha - 1.0):
            return x
    # Fallback — should never be reached for alpha >= 2.
    return 0.5


# ── Experiment kernel (with var_start support) ────────────────────────────


@njit
def _driftdm_dmc_vs_experiment_kernel(
    muc, b, non_dec, sd_non_dec, tau, a_shape, A, alpha,
    num_trials, sigma, dt, max_steps,
    var_non_dec, var_start, return_nan
):
    """Numba-compiled experiment-level DMC simulator with optional
    starting-point variability.

    Parameters
    ----------
    var_start : bool
        If True, each trial's starting point is drawn from
        Beta(alpha, alpha) and mapped to [-b, +b].
        If False, starting point is fixed at 0 (centre).
    """
    ...
    for i in range(num_trials):
        ...
        # Starting point
        if var_start:
            sp = _sample_symmetric_beta(alpha)          # in [0, 1]
            evidence = (2.0 * sp - 1.0) * b             # in [-b, +b]
        else:
            evidence = 0.0
        ...
        if var_non_dec:
            ndt = _draw_truncated_normal(
                non_dec, sd_non_dec,
                0.0, 2.0 * non_dec + 5.0 * sd_non_dec
            )
        else:
            ndt = non_dec
...
def driftdm_dmc_vs_experiment_simulator(
    muc: float, b: float, non_dec: float, sd_non_dec: float,
    tau: float, A: float, alpha: float,
    a_shape: float = 2.0, num_trials: int = 50,
    sigma: float = 1.0, dt: float = 0.0075,
    max_steps: int = 400,
    var_non_dec: bool = True, var_start: bool = True,
    return_nan: bool = False,
):
    num_trials = int(num_trials)
    max_steps = int(max_steps)
    rt, choice, condition = _driftdm_dmc_vs_experiment_kernel(
        muc, b, non_dec, sd_non_dec, tau, a_shape, A, alpha,
        num_trials, sigma, dt, max_steps,
        var_non_dec, var_start, return_nan,
    )
...
MODEL_REG_NAME = "driftdm_dmc_vs"
STORE_KEY_PREFIX = "dmc_vs"
DEFAULT_CHECKPOINT = "../../checkpoints/driftdm_dmc_vs"
_PARAM_KEYS = DRIFTDMC_DMC_VS_CONFIG["param_keys"]


def register():
    """Register the driftdm_dmc_vs simulator and config into global registries.
    ...
    simulators.TRIAL_SIMULATOR[MODEL_REG_NAME] = driftdm_dmc_vs_experiment_simulator
    default_settings.MODEL_CONFIG[MODEL_REG_NAME] = DRIFTDMC_DMC_VS_CONFIG
    default_settings.PARAMS_KEY_NAME_MAPPING = default_settings.get_param_mappings(
        default_settings.MODEL_CONFIG
    )

    # Create alias for downstream code
...
def get_dmc_vs_model(checkpoint_path=DEFAULT_CHECKPOINT):
    """Return a ready-to-use NSBICDM instance for driftdm_dmc_vs.
